...
namespace BookStore
{
    ...
    class LinkedListContainer<T> : IEnumerable<T> where T : IName<string>
    {
        ...
        public IEnumerable<T> Reverse()//у зворотному порядку зберігання;
        {
            Node current = tail;
            while (current != null)
            {
        ...
        public IEnumerable<T> GetBySubstring(string substring)//у імені елемента якого містяться заданий підрядок
        {
            Node current = head;
            while (current != null)
            {
        ...
        public IEnumerable<T> SortProductsByName()//у впорядкованому вигляді (порядок зберігання при цьому не повинен змінюватись).
        {
